...
OmicPro expects five logical CSV inputs. Their physical names and subdirectories
...
set `data.fold_column` explicitly. OmicPro rejects ambiguous multi-column files
instead of guessing.

The current external datasets were checked without copying them into the
repository. Public configs refer to a private mount through
`${OMICPRO_DATA_DIR}`; set that environment variable to the directory that
contains `Rice210`, `Maize368`, and `rape` before running a supplied config.
