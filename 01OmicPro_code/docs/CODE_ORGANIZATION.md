...
OmicPro separates five contracts that should remain independently testable. They
all live under `01OmicPro_code/`, while figure and application material is kept
at repository level in `02Figure_code/` and `03OmicPro_application/`.
...
- Baseline and ensemble models can be added through `model_registry.py`; only
  `omicpro` is registered in the current release.
